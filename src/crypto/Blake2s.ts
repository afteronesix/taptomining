...
class Blake2s {
  ...
  private compress(h: Uint32Array, m: Uint32Array, t: number, f: boolean) {
    ...
    for (let round = 0; round < 8; round++) {
      // NOTE: SIGMA has only 8 rounds, but original code used round % 10. We use round % 8 for safety/correctness.
      const s = this.SIGMA[round % 8];
  ...
  public hash(input: string): string {
    ...
    for (let i = 0; i < 8; i++) {
      const word = h[i];
      // Note: little-endian conversion is correct for Blake2s
...
export function TapcoinHash(input: string): string {
